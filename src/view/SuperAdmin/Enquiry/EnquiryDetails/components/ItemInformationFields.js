        ...
        <div className='grid md:grid-cols-3 gap-2'>
            <div className='col-span-1'>
                <FormItem
                    ...
                    invalid={errors?.accept_delivery_date && touched?.accept_delivery_date}
                    errorMessage={errors?.accept_delivery_date}
                >
                    <Field name='accept_delivery_date'>
                        {({ field, form }) => (
                            <DatePicker
                                style={(dayjs(values?.delivery_date).format('YYYY-MM-DD') !== dayjs(CalculateDate(values?.Product?.standard_lead_time, values?.Product?.standard_lead_time_type)).format('YYYY-MM-DD') && !values?.is_delivery_date_change) && { backgroundColor: `red`, color: 'white' }}
