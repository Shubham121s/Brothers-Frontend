...
import { toggleQuotationDialog } from "../store/stateSlice";
import { setSelectedEditQuotation } from "../store/dataSlice";
...
const QuotationDialog = () => {
  ...
  const editQuotationDialog = useSelector(
    (state) => state.quotation_detail.state.openDialog
  );

  const selectedQuotation = useSelector(
    (state) => state.quotation_detail.state.selectedQuotation
  );

  const onDialogClose = () => {
    dispatch(toggleQuotationDialog(false));
  };
  ...
  const calculateProfileValues = (values) => {
    ...
    const cutting_meter = values.cutting_mm * thickness_profile;
    const cutting_charges = cutting_meter * 4.5;

    // Calculating burning_charges
    const burning_charges =
      values.cutting_mm * thickness_profile * 3 * 0.00000785;

    // Calculating scrap_weight
    const scrap_weight = weight - values.finish_weight - burning_charges;
    ...
    const total_per_kg_rate = total_amount / values.finish_weight;
    ...
    const result = {
      ...
      finish_weight: parseFloat(values.finish_weight.toFixed(2)),
      cutting_mm: parseFloat(values.cutting_mm.toFixed(2)),
  ...
  const calculateFabricationValues = (values) => {
    const RawWeight = [];
    const ScrapWeight = [];
    const total_raw_weight =
      RawWeight?.reduce((acc, curr) => acc + parseFloat(curr.raw_weight), 0) ||
      0;
    const total_raw_amount =
      RawWeight?.reduce((acc, curr) => acc + parseFloat(curr.amount), 0) || 0;

    const total_scrap_weight =
      ScrapWeight?.reduce((acc, curr) => acc + curr.raw_weight, 0) || 0;
    const total_scrap_amount =
      ScrapWeight?.reduce((acc, curr) => acc + curr.amount, 0) || 0;
    ...
    const data = {
      ...
      scrap_weight_and_amount: [],
      raw_weight_and_amount: [],
    };
      ...
      <Formik
        ...
        onSubmit={(values, { resetForm, setSubmitting }) => {
          ...
          if (values.part_type === "PROFILE DRAWING") {
            result = calculateProfileValues({ ...values });
          }
          ...
          dispatch(setSelectedEditQuotation(result));
          dispatch(toggleQuotationDialog(false));
          result = {};
          resetForm();
        }}
