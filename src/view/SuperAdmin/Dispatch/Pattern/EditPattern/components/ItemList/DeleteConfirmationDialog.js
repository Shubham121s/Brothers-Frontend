...
import { deletePatternProductToInvoice } from "../../store/dataSlice";
...
const DeleteProductConfirmationDialog = ({ fetchData }) => {
  ...
  const deleteDispatchProductDialog = useSelector(
    (state) => state.edit_pattern_dispatch.state.deleteConfirmation
  );
  ...
  const onDelete = async () => {
    ...
    const action = await dispatch(
      deletePatternProductToInvoice({
        pattern_invoice_list_id: selectedDispatch?.pattern_invoice_list_id,
      })
    );
    setLoading(false);
