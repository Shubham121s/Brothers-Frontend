...
const InvoiceDialog = () => {
    ...
    const invoiceDialog = useSelector(state => state.dispatch_invoice.state.invoiceDialog)
    const selectedInvoice = useSelector(state => state.dispatch_invoice.state.selectedInvoice)
    ...
    return (
        ...
        >
            {
                selectedInvoice?.invoice_type === 'domestic' ? <div className="flex flex-col h-full justify-between">
                    <h3 className='mb-4 text-center'>Print Domestic Invoice</h3>
                    <DomesticInvoice dispatch_invoice_id={selectedInvoice?.dispatch_invoice_id} />
                </div> : <div className="flex flex-col h-full justify-between">
