...
const ActionColumn = (props) => {
    const { dispatchList, handleDeleteItemInPoList, setFieldValue, locationIndex, index } = props
    const onDelete = () => {
        handleDeleteItemInPoList?.(dispatchList, locationIndex, setFieldValue, index)
    }
...
const ItemTable = ({ initialData = [], locationIndex, dispatchList, handleDeleteItemInPoList, setFieldValue }) => {
                ...
                columns: [
                    {
                        ...
                        cell: (props) => {
                            ...
                            return (
                                <ActionColumn index={index} locationIndex={locationIndex} setFieldValue={setFieldValue} dispatchList={dispatchList} handleDeleteItemInPoList={handleDeleteItemInPoList} />
                            )
                        },
                    }
                ],
                    ...
                    {table.getHeaderGroups().map((headerGroup) => (
                        <Tr key={headerGroup.id}>
                            {headerGroup.headers.map((header) => {
                                return (
                                    <Th
                                        style={{ border: '.5px solid #e0e0e0', textAlign: 'center' }}
                        ...
                        return (
                            <Tr key={row.id}>
                                {row.getVisibleCells().map((cell) => {
                                    return (
                                        <Td key={cell.id} style={{ border: '.5px solid #e0e0e0' }} className='select-none'>
                                            {flexRender(
