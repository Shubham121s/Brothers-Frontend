...
import { getAllPosByCustomerId, getAllShippingAddressByCustomerId } from '../../NewDispatch/store/dataSlice';
...
const ConsigneeInformationFields = (props) => {
    ...
    const fetchPoList = useCallback(async () => {
        ...
        if (values) {
            dispatch(getAllPosByCustomerId({ customer_id: values?.customer_id, currency_type: 'INR' }))
