...
const ItemQuantityInformationFieldsForm = (props) => {
  const { errors, values, touched, poList, dispatchList } = props;
  const [value, setValue] = useState(values || 0);
  const [pendingQuantity, setPendingQuantity] = useState(
    parseInt(poList?.quantity) -
      parseInt(
        getAllPoList(dispatchList)
          ?.filter((item) => item?.PoList?.po_list_id === poList?.po_list_id)
          ?.reduce((sum, po) => sum + parseInt(po?.quantity), 0)
      ) || 0
  );
          ...
          <div className="grid grid-cols-4 gap-1">
            ...
            <div className="col-span-2 grid grid-cols-2 gap-1">
              ...
              <Input
                ...
                onChange={(event) => {
                  if (
                    event.target.value > pendingQuantity ||
                    event.target.value === "" ||
                    event.target.value < 0
                  ) {
                    setValue(0);
                    return form.setFieldValue(field.name, 0);
                  }
...
export default memo(ItemQuantityInformationFieldsForm);
