...
const ItemQuantityInformationFields = (props) => {
    ...
    useEffect(() => {
        setPendingQuantity(selectedDispatchItem?.PoList?.quantity - getAllPoList(dispatchList)?.filter((item) => item?.PoList?.po_list_id === selectedDispatchItem?.PoList?.po_list_id)?.reduce((sum, list) => sum + parseInt(list?.item_quantity), 0))
                ...
                {({ field, form }) => (
                    <div className='grid grid-cols-4 gap-1'>
                        <div className='col-span-1'>
                            <Button className='w-full' style={{ borderRadius: '5px 0px 0px 5px' }} type='button' icon={<HiMinus />} disabled={value === 1 || value === ''} onClick={() => {
                        ...
                        <div className='col-span-2 grid grid-cols-2 gap-1'>
                            ...
                            <Card className='flex justify-center items-center' bodyClass='p-0 flex justify-center item-center'>{value}</Card>
                        </div>
                        <div className='col-span-1'>
                            <Button className='w-full' style={{ borderRadius: '0px 5px 5px 0px' }} disabled={pendingQuantity === 0} icon={<HiPlus />} type='button' onClick={() => {
