...
const DispatchInvoiceTable = () => {
  ...
  const data = useSelector(
    (state) => state.pattern_invoice.data.dispatchInvoiceList
  );
  const loading = useSelector((state) => state.pattern_invoice.data.loading);
  const { type } = useSelector(
    (state) => state.pattern_invoice.data.filterData
  );

  const { pageIndex, pageSize, sort, query, total, customer_id } = useSelector(
  ...
  }, [pageIndex, pageSize, sort, query, type, dispatch, customer_id]);

  useEffect(() => {
    fetchData();
  }, [fetchData, pageIndex, pageSize, sort, type, customer_id]);
  ...
  const tableData = useMemo(
    () => ({ pageIndex, pageSize, sort, query, total, customer_id }),
    [pageIndex, pageSize, sort, query, total, customer_id]
  );
