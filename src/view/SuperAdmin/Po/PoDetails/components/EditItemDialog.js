...
import { Button, Card, Dialog, FormContainer, Notification, Toast } from '../../../../../components/ui'
...
const EditItemDialog = forwardRef((props, ref) => {
  ...
  const handleUpdatePoList = async (values, setSubmitting) => {
    ...
    if (action.payload?.status === 200) {
      onDialogClose()
      return pushNotification(action.payload?.data?.message, 'success', 'Successfully Updated')
    }
  ...
  return (
    ...
    >
      ...
      >
        {({ values, touched, errors, isSubmitting, setFieldError, setFieldTouched }) => (
          <Form>
            <FormContainer>
              <h4>Item Details Information</h4>
              <p className="mb-2">Section to config item details information</p>
              <div className='grid grid-cols-2 gap-2 mb-3'>
                {!isEmpty(values) ? <Card className='mt-2 bg-slate-50'>
                  ...
                  <div className='flex justify-between'><strong>Product Name :</strong> <span>{values?.Product?.name || '-'}</span></div>
                  <div className='flex justify-between'><strong>Item Code :</strong> <span>{values?.Product?.item_code || '-'}</span></div>
                ...
                {!isEmpty(values) ? <Card className='mt-2 bg-blue-50'>
                  <div className='flex justify-between'><strong>Quantity ({values?.Product?.unit_measurement}):</strong> <span>{values?.quantity || '-'}</span></div>
                  <div className='flex justify-between'><strong>Price ({currency}):</strong> <span>{values?.unit_price || '-'}</span></div>
                  <div className='flex justify-between'><strong>Amount ({currency}):</strong> <span>{(
                    (values?.unit_price * values?.quantity)
                  ).toFixed(2) || '-'}</span></div>
                  <div className='flex justify-between'><strong>Delivery Date :</strong> <span>{dayjs(values?.delivery_date).format('DD-MMM-YYYY') || '-'}</span></div>
                  <div className='flex justify-between'><strong>Remark :</strong> <span>{values?.description || '-'}</span></div>
                </Card> : null
                }
              </div>
              {(values?.list_status === 'accepted' || values?.list_status === 'rejected') ?
                <div className='grid md:grid-cols-3 mb-3'>
                  <div className='col-span-2'>
                    <Card className={`${statusColor[values?.list_status]?.bgClass}`}>
                      <div className='flex justify-between'><strong>Status :</strong> {
                        <span className={`ml-2 font-semibold capitalize ${statusColor[values?.list_status]?.textClass}`}>
                          {statusColor[values?.list_status]?.label}
                        </span> || '-'
                      }</div>
                      <div className='flex justify-between'><strong>Brother Delivery Date :</strong> <span>{dayjs(values?.accept_delivery_date).format('DD-MMM-YYYY') || '-'}</span></div>
                      <div className='flex justify-between'><strong>Brother Remark :</strong> <span>{values?.accept_description || '-'}</span></div>
                    </Card>
                  </div>
                </div>
              ...
              <div className="flex justify-end px-5 py-2 bg-gray-100 gap-2 rounded-bl-lg rounded-br-lg">
                <Button size='sm' type='button' variant='' onClick={onDialogClose}>Discard</Button>
                {
                  (values?.list_status === 'accepted' || values?.list_status === 'rejected') ? null : <>
                    <Button
