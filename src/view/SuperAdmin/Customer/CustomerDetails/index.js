...
import { apiDeleteCustomer, apiUpdateCustomerDetails } from '../../../../services/SuperAdmin/Customer/CustomerService';
...
import { setCustomerId, toggleNewShippingAddressDialog, toggleNewShippingDetailsDialog } from './store/stateSlice';
...
const CustomerDetails = () => {
    ...
    const data = useSelector(state => state.customer_details.data.customerDetails)
    ...
    const handleFormSubmit = async (values, setSubmitting) => {
        ...
        if (response.data?.success) {
            popNotification('Updated', (String(data.customer_type).charAt(0).toUpperCase() + String(data.customer_type).slice(1)))
        }
    }
    ...
    const handleDelete = async ({ setDialogOpen, setLoading }) => {
        ...
        if (response.data?.success) {
            setDialogOpen(false)
            popNotification('Deleted', (String(data.customer_type).charAt(0).toUpperCase() + String(data.customer_type).slice(1)))
        }
        setLoading(false)
    }
                        ...
                        <Card className='mt-4 bg-gray-50'>
                            <Tabs defaultValue="address">
                                <TabList className='flex justify-between'>
                                    <div className='flex'>
                                        <TabNav disabled value="order">PURCHASE ORDERS</TabNav>
                                        <TabNav disabled value="invoice">INVOICES</TabNav>
                                        <TabNav value="address">SHIPPING ADDRESSES</TabNav>
                                        <TabNav value="details">SHIPPING DETAILS</TabNav>
                                    </div>
                                    <div>
                                        <TabContent value='address'>
                                            <Button onClick={onAddShippingAddress} size='sm' variant='solid' color='pink-500'>New Address</Button>
                                        </TabContent>
                                        <TabContent value='details'>
                                            <Button onClick={onAddShippingDetails} size='sm' variant='solid' color='pink-500'>New Shipping</Button>
                                        </TabContent>
                                    </div>
                                </TabList>
                                <div className="p-4">
